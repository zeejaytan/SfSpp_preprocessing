...
build_base_container() {
    ...
    log_info "This includes: PCL, CGAL, VTK from Ubuntu 22.04 repos"
    
    if timeout 1800 "${APPTAINER_BIN}" build --fakeroot "${base_container}" sfs_prep_base.def > "${log_file}" 2>&1; then
...
build_preprocessing_container() {
    local base_container="${CACHE_DIR}/sfs_prep_base.sif"
    local preprocessing_container="${BUILD_DIR}/sfs_preprocessing.sif"
    ...
    sed "s|From: sfs_prep_base.sif|From: ${base_container}|" sfs_prep_tools.def > "${temp_def}"
...
test_container() {
    local container="${BUILD_DIR}/sfs_preprocessing.sif"
    ...
    if "${APPTAINER_BIN}" exec "${container}" /bin/bash -c "
        ...
        ls -la /opt/sfs_prep/bin/ 2>/dev/null && echo '✓ SfS preprocessing tools directory exists'
...
main() {
    log_info "Starting SfS Preprocessing build pipeline..."
    ...
    log_info "Container available at: ${BUILD_DIR}/sfs_preprocessing.sif"
    log_info "Usage examples:"
    log_info "  apptainer exec sfs_preprocessing.sif MeshPreprocessing"
    log_info "  apptainer exec sfs_preprocessing.sif EdgeLineExtraction"
