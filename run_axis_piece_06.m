...
try
    fprintf('Starting axis extraction for Pot A, Piece 06\n');
    
    % Call the main axis extraction function
    vt = extract_axis('A', 06, false);
    
    % Save results
    output_file = sprintf('%s/Pot_A_Piece_06_Axis.txt', output_dir);
    writematrix(vt, output_file, 'Delimiter', ' ');
    
    fprintf('✅ Axis extraction completed for Piece 06\n');
