...
// Custom function to save PCD with SFS-compatible segment headers
void savePCDFileWithSegmentHeaders(const std::string& filepath, 
    ...
    // Calculate segments (5 segments like original format)
    int total_points = cloud.size();
    int num_segments = std::min(5, std::max(1, total_points / 50));
    int points_per_segment = total_points / num_segments;
    
    // Write custom header with segment information
    file << "# .PCD v0.7 - Point Cloud Data file format\n";
    file << "# " << num_segments << " " << total_points << " 0\n";
    
    // Write segment range headers
    for (int seg = 0; seg < num_segments; seg++) {
        int start_idx = seg * points_per_segment + 1;  // 1-based indexing
        int end_idx = (seg == num_segments - 1) ? total_points : (seg + 1) * points_per_segment;
        file << "# " << start_idx << " " << end_idx << " 0\n";
    }
    ...
    // Write point data with normals and curvature
    ...
    file.close();
    std::cout << "Saved PCD with custom segment headers: " << filepath 
              << " (" << num_segments << " segments, " << total_points << " points)" << std::endl;
}
...
// NURBS includes replaced with modern TPS surface fitting
...
public:
    ...
    SurfacePoint projectPointOntoSurface(const Eigen::Vector3d& originalPt, 
        ...
        double current_x = min_x + hintPt(0) * (max_x - min_x);
        double current_y = min_y + hintPt(1) * (max_y - min_y);
        
        // Newton-Raphson optimization (exact NURBS inverse mapping equivalent)
        ...
        for (int iteration = 0; iteration < max_steps; iteration++) {
            ...
            // Newton-Raphson step using surface derivatives
...
public:
    ...
    void buildFromPointCloud(pcl::PointCloud<Point>::Ptr surface_cloud) {
...
// Global path variables - will be initialized in main()
std::string datasetBreaklinesFolder;
std::string datasetSurfacesFolder;
std::string tempEdgeFolder;
std::string datasetPath_global;
...
//--------------------------- REMOVED: old bspline curve conversion (replaced with TPS) ---------------------------------//
...
void fitBSplineSurfaceAndGetNormalsOnProjectedPoints(string pointCloudDataFile, pcl::PointCloud<Point>::Ptr sampledBreaklinePointCloud, pcl::PointCloud<PointNormal>::Ptr projectedPointCloudWithNormals)
{
	...
	// Skip visualization in headless/container environments to prevent segfault
	bool enable_visualization = false;  // Set to true only when X11 display available
	pcl::visualization::PCLVisualizer viewer("TPS surface fitting");
	...
	// TPS surface data (replacing NURBS data)
	...
	// TPS builds directly from point cloud - no separate data conversion needed
	...
	// ############################################################################
	// Modern TPS surface fitting (replacing B-spline surface)

	// TPS fitting parameters (equivalent to NURBS parameters)
	unsigned order(3);  // Preserved for compatibility
	unsigned refinement(3);
	unsigned iterations(8);
	unsigned mesh_resolution(128);

	// TPS parameters (equivalent to NURBS smoothness)
	double interior_smoothness = 0.2;
	double interior_weight = 1.0;
	double boundary_smoothness = 0.4;
	double boundary_weight = 0.0;

	printf("  TPS surface fitting ...\n");

	// TPS surface fitting initialization (replacing NURBS)
	printf("  TPS surface building...\n");
	
	// Build TPS surface directly from point cloud
	projector.buildFromPointCloud(cloud);
	
	// Create visualization mesh from TPS (optional - for viewer compatibility)
	pcl::PolygonMesh mesh;
	pcl::PointCloud<pcl::PointXYZ>::Ptr mesh_cloud(new pcl::PointCloud<pcl::PointXYZ>);
	std::vector<pcl::Vertices> mesh_vertices;
	std::string mesh_id = "mesh_tps";
	
	// Generate mesh from TPS surface for visualization
	for (int u = 0; u < mesh_resolution; u++) {
		for (int v = 0; v < mesh_resolution; v++) {
			double x = cloud->points[0].x + (double)u / mesh_resolution * (cloud->points.back().x - cloud->points[0].x);
			double y = cloud->points[0].y + (double)v / mesh_resolution * (cloud->points.back().y - cloud->points[0].y);
			pcl::PointXYZ pt;
			pt.x = x;
			pt.y = y;
			pt.z = 0;  // Will be updated with TPS evaluation
			mesh_cloud->points.push_back(pt);
		}
	}
	mesh_cloud->width = mesh_cloud->points.size();
	mesh_cloud->height = 1;
	if (enable_visualization) {
		viewer.addPointCloud<pcl::PointXYZ>(mesh_cloud, "tps_surface");
	}

	// TPS surface is fitted in one step - no iterative refinement needed
	// (TPS provides inherent smoothness and optimal fit)
	std::cout << "TPS surface fitted with C2 continuity" << std::endl;
	
	// Update visualization (optional)
	if (enable_visualization) {
		for (unsigned i = 0; i < refinement; i++) {
			std::cout << "TPS visualization update " << (i+1) << "/" << refinement << std::endl;
			viewer.spinOnce();
		}
	}
	
	// TPS final state (already optimal)
	std::cout << "TPS surface fitting completed" << std::endl;
	if (enable_visualization) {
		for (unsigned i = 0; i < iterations; i++) {
			std::cout << "TPS final state " << (i+1) << "/" << iterations << std::endl; 
			viewer.spinOnce();
		}
		viewer.close();
	}
	...
	for (size_t i = 0; i < sampledBreaklinePointCloud->points.size(); i++)
	{
		...
		ptTmp.x = sampledBreaklinePointCloud->points[i].x;
		ptTmp.y = sampledBreaklinePointCloud->points[i].y;
		ptTmp.z = sampledBreaklinePointCloud->points[i].z;
		ptTmp.normal_x = surface_point.normal.x();  // TPS analytical normal
	...
	for (size_t i = 0; i < cloud_OriginalPointsOnSurface->points.size(); i++)
	{
		...
		Eigen::Vector2d hintPt(0.5, 0.5);
		// TPS surface projection (replacing inverse mapping)
		double error;
		int im_max_steps = 100;
		double im_accuracy = 1e-5;
		...
		// TPS analytical normal (equivalent to NURBS EvNormal)
		Eigen::Vector3d normalEst = surface_point.normal;
		...
		ptTmp.normal_x = -normalEst.x();  // TPS analytical normal
...
//----------------------------------------------------------------------------
// #include <pcl/surface/on_nurbs/fitting_curve_pdm.h>  // NURBS curve fitting removed - not used
...
void detectSeparateLineSegments(string b1_FilePath, int len = 10) //len = 10
{
	...
	// Skip visualization in headless/container environments to prevent segfault
	bool enable_line_seg_visualization = false;  // Set to true only when X11 display available
	if (enable_line_seg_visualization) {
...
void processFragmentData(string surfacePointCloudFilePath, string fragmentMeshFilePath)
{
    ...
    typedef vector<std::filesystem::path> vec;
    vec v;  // "Segments" 폴더 내 파일들을 읽음
    std::filesystem::path p("Segments/");
	...
	savePCDFileWithSegmentHeaders(pcdFilePath, *cloud_CompleteBreakline);
    ...
    segCount = 1;
    typedef vector<std::filesystem::path> vecB;
    vecB vB;
    copy(std::filesystem::directory_iterator(p), std::filesystem::directory_iterator(), back_inserter(vB));
    ...
    for (vecB::const_iterator it(vB.begin()), it_end(vB.end()); it != it_end; ++it)
    {
        ...
        // Call fracture surface processing for this segment
        pcl::PointCloud<pcl::PointNormal>::Ptr cloud_SegmentFracturedSurface(new pcl::PointCloud<pcl::PointNormal>);
        pcl::PointCloud<pcl::PointNormal>::Ptr cloud_SegmentIntExtSurface(new pcl::PointCloud<pcl::PointNormal>);
        
        getPointsOnFracturedSurface(cloud_breakLineSeg, fragmentMeshFilePath, 
                                   outPathTemp + fileNameOnly + "_SampledWithNormals.ply", 
                                   segCount, cloud_SegmentFracturedSurface, cloud_SegmentIntExtSurface);
        
        // Add segment fracture surface points to overall fracture surface
        for (const auto& point : cloud_SegmentFracturedSurface->points) {
            cloud_PointsOnFracturedSurface->points.push_back(point);
        }
        for (const auto& point : cloud_SegmentIntExtSurface->points) {
            cloud_PointsOnIntExtSurfaceNearBreakline->points.push_back(point);
        }
    ...
    if (!vectorPointNormal.empty())
    {
        ...
        // Also save as Surface_F.pcd format for SFS main system compatibility
        std::string surfaceFFileName = currentFileName;
        size_t posSurface0 = surfaceFFileName.find("_Surface_0");
        size_t posSurface1 = surfaceFFileName.find("_Surface_1");
        if (posSurface0 != std::string::npos) {
            surfaceFFileName.replace(posSurface0, std::string("_Surface_0").length(), "_Surface_F");
        } else if (posSurface1 != std::string::npos) {
            surfaceFFileName.replace(posSurface1, std::string("_Surface_1").length(), "_Surface_F");
        }
        
        if (!cloud_PointsOnFracturedSurfaceNoDuplicates->empty()) {
            std::cout << "  Saving fracture surface file: " << surfaceFFileName << ".pcd" << std::endl;
            pcl::io::savePCDFile(surfaceFFileName + ".pcd", *cloud_PointsOnFracturedSurfaceNoDuplicates);
        }
...
int main(int argc, char* argv[])
{
    ...
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " <pot_id> <num_pieces>" << std::endl;
        std::cerr << "Example: " << argv[0] << " A 8" << std::endl;
        return 1;
    }
    
    std::string potID = argv[1];
    int numPieces = std::stoi(argv[2]);
    
    std::cout << "Processing Pot " << potID << " with " << numPieces << " pieces" << std::endl;

    // Initialize global path variables
    datasetBreaklinesFolder = getBreaklineDatasetPath(potID);
    datasetSurfacesFolder = getSurfaceDatasetPath(potID);
    tempEdgeFolder = tempEdgePath(potID);
    datasetPath_global = tempDataPath(potID);
