...
% Extract axis for a single NURBS piece number
...
if ~exist('original_nurbs_preprocessing/AxisExtraction', 'dir')
    mkdir('original_nurbs_preprocessing/AxisExtraction');
end

% Define file paths for NURBS surfaces
surface_0_file = sprintf('original_nurbs_preprocessing/Temp/Data/Pot_A/Pot_A_Piece_%02d_Surface_0.xyz', piece_num);
surface_1_file = sprintf('original_nurbs_preprocessing/Temp/Data/Pot_A/Pot_A_Piece_%02d_Surface_1.xyz', piece_num);
axis_output_file = sprintf('original_nurbs_preprocessing/AxisExtraction/Pot_A_Piece_%02d_NURBS_Axis.xyz', piece_num);
...
try
    ...
    fprintf('  🔄 Running PotSAC axis extraction on NURBS data...\n');
    ...
    if size(axis_candidates, 1) >= 6 && size(axis_candidates, 2) >= 1
        ...
        fprintf('  ✅ PotSAC extracted %d NURBS axis candidates\n', num_candidates);
