...
// typedef pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> ColorHandlerXYZ;  // Headless build
...
// pcl::visualization::PCLVisualizer viewer_FinalResult("PCL Reg Visualizer");  // Headless build
// pcl::visualization::PCLVisualizer::Ptr viewer_New(new pcl::visualization::PCLVisualizer("Check breakline smoothing"));  // Headless build


// Custom function to save PCD with proper geometric segmentation headers
void savePCDFileWithSegmentHeaders(const std::string& filepath, 
    ...
    // Write segment headers - this is the key addition for proper geometric segmentation
    file << "# " << segmentIndex.size() << " " << cloud.points.size() << " 0\n";
    ...
    file.close();
    std::cout << "Saved PCD with geometric segmentation headers: " << filepath 
              << " (" << segmentIndex.size() << " segments, " << cloud.points.size() << " points)" << std::endl;
}

// Headless build - visualization functions disabled
/*
...
void fitBSplineSurfaceAndGetNormalsOnProjectedPoints(string pointCloudDataFile, pcl::PointCloud<Point>::Ptr sampledBreaklinePointCloud, pcl::PointCloud<PointNormal>::Ptr projectedPointCloudWithNormals)
{
	...
	// pcl::visualization::PCLVisualizer viewer("B-spline surface fitting");  // Headless build
	// viewer.setSize(800, 600);  // Headless build
	...
	// pcl::visualization::PointCloudColorHandlerCustom<Point> handler(cloud, 0, 255, 0);  // Headless build
	// viewer.addPointCloud<Point>(cloud, handler, "cloud_cylinder");  // Headless build
	...
	// viewer.addPolygonMesh(mesh, mesh_id);  // Headless build
	...
	for (unsigned i = 0; i < refinement; i++)
	{
		...
		// viewer.updatePolygonMesh<pcl::PointXYZ>(mesh_cloud, mesh_vertices, mesh_id);  // Headless build
		// viewer.spinOnce();  // Headless build
	}
...
// pcl::visualization::PCLVisualizer viewerT("Curve Fitting 3D");  // Headless build
...
void detectSeparateLineSegments(string b1_FilePath, int len = 10) //len = 10
{
	...
	// Headless build - visualization disabled
	/*
...
void processFragmentData(string surfacePointCloudFilePath, string fragmentMeshFilePath)
{
	...
	// Save PCD file with proper geometric segmentation headers
	savePCDFileWithSegmentHeaders(pcdFilePath, *cloud_CompleteBreakline, index);
