...
# Copy only required sources for EdgeLineExtraction
mkdir -p container_build
cp -r edgeline_extraction.cpp data_path.h alglib/ tiny_obj_loader.h container_build/ 2>/dev/null || echo "Some files may not exist"

# Create a container-compatible CMakeLists.txt (placed as CMakeLists.txt)
cat > container_build/CMakeLists.txt << 'EOF'
...
# Build only EdgeLineExtraction (no MeshPreprocessing needed here)
...
add_custom_target(all_targets ALL DEPENDS EdgeLineExtraction)
EOF
...
$APPTAINER exec --bind $PWD:/workspace --pwd /workspace "$CONTAINER_PATH" /bin/bash -c "
    ...
    cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS='-O2 -DNDEBUG -Wno-old-style-cast -Wno-deprecated-declarations'
    ...
    # Check results
    echo 'Build results:'
    ls -la EdgeLineExtraction 2>/dev/null || echo 'EdgeLineExtraction missing'
...
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    ...
    if [ -f "container_build/build/EdgeLineExtraction" ]; then
        cp container_build/build/EdgeLineExtraction ./
        echo "✓ EdgeLineExtraction built successfully!"
        echo "Available executables:"
        ls -la EdgeLineExtraction
    else
