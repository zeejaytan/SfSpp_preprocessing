...
# Build SFS Preprocessing Container (Ubuntu 20.04 - Working Version)
echo "=== Building SFS Preprocessing Container (Ubuntu 20.04 - Working Version) ==="

# Load modules
module load Apptainer/1.3.3

# Configuration
CONTAINER_NAME="sfs_prep_ubuntu20.sif"
DEFINITION_FILE="sfs_prep_ubuntu20.def"
BUILD_LOG="logs/sfs_prep_ubuntu20_build.log"
...
if [ ${PIPESTATUS[0]} -eq 0 ] && [ -f "cache/$CONTAINER_NAME" ]; then
    ...
    apptainer exec cache/$CONTAINER_NAME /bin/bash -c "
        ...
        find /usr/local -name '*vtk*' -type d 2>/dev/null | head -3
        find /usr/local -name '*pcl*' -type d 2>/dev/null | head -3
