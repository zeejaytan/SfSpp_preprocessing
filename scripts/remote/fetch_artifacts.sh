#!/usr/bin/env bash
# Rsync a relative path from the Spartan sfs_preprocessing tree to a local directory.
# Usage: ./scripts/remote/fetch_artifacts.sh <remote-relpath> [local-dir]
set -euo pipefail

SPARTAN_HOST="${SPARTAN_HOST:-spartan}"
REMOTE_ROOT="${REMOTE_ROOT:-/data/gpfs/projects/punim2657/sfs_preprocessing}"
